...
export default function HeroSlider() {
  ...
  const safeIndex = images.length > 0 && index < images.length ? index : 0;
  ...
  return (
    <div className="absolute inset-0 overflow-hidden bg-blue-50">
      ...
      {images.length > 0 && (
        <div 
          className="absolute inset-0 bg-cover bg-center blur-3xl opacity-60 scale-125 transition-all duration-1000"
      ...
      <AnimatePresence>
        ...
        >
          ...
          {/* 1. Base Contrast */}
          <div className="absolute inset-0 bg-gradient-to-br from-blue-600/10 to-indigo-500/10 mix-blend-overlay" />
          
          {/* 2. Soft Vignette to draw focus */}
          <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_center,_var(--tw-gradient-stops))] from-transparent via-transparent to-blue-900/20" />
          
          {/* 3. Bottom Gradient for smooth transition to the gray-50 background of the app */}
          <div className="absolute inset-0 bg-gradient-to-t from-gray-50 via-blue-900/5 to-transparent opacity-100" />
          
          {/* 4. Top Gradient to protect the white header text */}
          <div className="absolute inset-0 bg-gradient-to-b from-blue-900/40 via-transparent to-transparent opacity-80" />
      ...
      <div className="absolute bottom-10 left-1/2 -translate-x-1/2 flex gap-2 z-20">
        {images.map((_, i) => (
          <div 
            key={i} 
            className={`h-1 rounded-full transition-all duration-500 ${i === safeIndex ? 'w-8 bg-blue-600' : 'w-2 bg-gray-300'}`}
          />
        ))}
      </div>
    </div>
